...
export class SignalRService {
    ...
    public renewConnection(): void {
        ...
        this.httpClient.get<SignalRConnection>(environment.PWASignalRConnectionUrl).pipe(tap((value: SignalRConnection) => {
            this.signalRConnection = new HubConnectionBuilder().withUrl(value.url, { accessTokenFactory: () => value.accessToken }).build();
            this.signalRConnection.on("SignalRSubscriptionCountEvent", (value: { Count: number }) => {
            ...
            this.signalRConnection.start().then(() => console.log('SignalR Connected!')).catch((error) => {
                console.error(error);
            });

        }), catchError((error: any) => {
            return ErrorManager.generalError("SignalRService.constructor", JSON.stringify(error));
        })).subscribe((value: SignalRConnection) => { });
    }
