...
function createInitialState(): AuthenticationState {
  let authenticationInformation: Authentication =
    StorageManager.get<Authentication>(TokenConstant.token);
...
function createAuthenticationState(
  ...
  StorageManager.add<Authentication>(
    TokenConstant.token,
...
export class AuthenticationStore extends Store<AuthenticationState> {
  ...
  refreshToken(token: string): void {
    const authenticationInformation: Authentication =
      StorageManager.get<Authentication>(TokenConstant.token);
