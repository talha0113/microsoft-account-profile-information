...
export class ProfileService {
    ...
    get information(): Observable<Profile> {
        ...
        return forkJoin({ basicInformation, profilePicture }).pipe(map((values: { basicInformation: Profile, profilePicture: Blob }) => {
            if (values.basicInformation == null && values.profilePicture == null) {
                return null;
            }
            else {
                let profileInformation: Profile = values.basicInformation;
